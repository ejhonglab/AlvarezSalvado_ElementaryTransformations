...
function out = importer2 (varargin)
...
%         disp([refs(i).date, ' going old']);
    ...
    for j = 1:length(Stimulinames); % Iterates through each different stimulus
         ...
         % TO IMPORT DATA PREVIOUS TO JULY 2017 USE THE FOLLOWING LINE INSTEAD
%          X.(name) = eval(['flyseparator_prejul2017 (s', Stimulinames{j}, ');']);
