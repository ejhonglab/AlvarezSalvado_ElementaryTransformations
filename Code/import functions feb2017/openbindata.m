...
space = 5 + nflies*3; % Calculates the number of rows that each iteration uses

% Saves the data in the output structure
out.time = file(2:space:end);
out.stimulus = file(3:space:end);
out.compstimulus = file(4:space:end);
out.analog = file(5:space:end);
for i = 1:nflies
